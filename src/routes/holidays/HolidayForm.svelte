<script lang="ts">
	...
	let { holiday = $bindable(), onsubmit, children }: Props = $props()
</script>

<form {onsubmit}>
	<div class="group">
		<FormGroup id="employeeId" label="Personal">
			<EmployeePicker bind:value={holiday.employeeId} />
		</FormGroup>
	</div>
	<div class="group">
		<FormGroup id="startDate" label="Fecha de Inicio">
			<DateInput id="startDate" bind:value={holiday.startDate} />
		</FormGroup>
		<FormGroup id="endDate" label="Fecha de Fin">
			<DateInput id="endDate" bind:value={holiday.endDate} />
		</FormGroup>
